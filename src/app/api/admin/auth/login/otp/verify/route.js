...
export async function POST(req) {
  try {
    ...
    if (signInError) {
      console.error('Supabase signInWithPassword failed on admin verify', signInError.message || signInError);
    ...
    const admin = await prisma.adminUser.findUnique({ where: { id: otp.adminUserId } });
    return Response.json({ session: data.session, admin: { id: admin?.id, name: admin?.name, email: admin?.email } });
  } catch (err) {
    console.error(err);
