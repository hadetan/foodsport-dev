...
import { getById, getMany, insert, updateById } from "@/lib/prisma/db-utils";
...
function parseQueryParams(searchParams) {
    return {
        ...
        limit: parseInt(searchParams.get("limit") || "10000", 10), // High limit to fetch all activities
    ...
    try {
        ...
        for (const { activityId } of participantCountsRaw) {
            participantCountMap[activityId] =
                (participantCountMap[activityId] || 0) + 1;
        }

        const organizerIds = Array.from(
            new Set(
                (activities || []).map((a) => a.organizerId).filter(Boolean)
            )
        );
        let organizerNameMap = {};
        if (organizerIds.length > 0) {
            const organizers = await getMany(
                "adminUser",
                { id: { in: organizerIds } },
                { id: true, name: true }
            );
        ...
        const activitiesList = (activities || []).map((a) => {
            return {
                ...
                organizerName: a.organizerId
                    ? organizerNameMap[a.organizerId]
                    : "unknown",
                ...
                tncs: (a.tncs || []).map((t) => ({
                    ...
                    createdBy: t.adminUserId
                        ? adminMap[t.adminUserId] || null
                        : null,
                    updatedBy: t.updatedBy
                        ? adminMap[t.updatedBy] || null
                        : null,
...
async function buildActivityResponse(id) {
    ...
    // participant count
    const participantRows = await getMany(
        "userActivity",
        { activityId: id },
        { activityId: true }
    );
    const participantCount = Array.isArray(participantRows)
    ...
    if (activity.organizerId) {
        ...
        if (Array.isArray(admins) && admins.length > 0)
            organizerName = admins[0].name || organizerName;
    }

    const tncsRaw = Array.isArray(activity.tncs) ? activity.tncs : [];
    const adminIds = Array.from(
        new Set(
            tncsRaw.flatMap((t) => [t.adminUserId, t.updatedBy]).filter(Boolean)
        )
    );
    let adminMap = {};
    if (adminIds.length > 0) {
...
export async function POST(req) {
    try {
        ...
        for (const field of allowedFields) {
            // Skip imageUrl and bannerImageUrl as they're set from file uploads, not form fields
            if (field === "imageUrl" || field === "bannerImageUrl") continue;
        ...
        if (activityData.participantLimit) {
            activityData.participantLimit = Number(
                activityData.participantLimit
            );
        ...
        if (
            ...
            isNaN(Date.parse(activityData.startDate))
        ) {
            return NextResponse.json(
        ...
        const validation = validateRequiredFields(activityData, [
            "title",
            "activityType",
            "location",
            "startDate",
        ...
        if (activity && activity.error) {
            return NextResponse.json(
                {
                    error: formatDbError(activity.error),
        ...
        const canonical = await buildActivityResponse(
            activity.id || activity?.[0]?.id || activity?.id
        );
        ...
        return new NextResponse(
            JSON.stringify({
                error: "Failed to create activity. Please try again later.",
                message: error.message,
            }),
            {
                status: 500,
    ...
    try {
        ...
        if (newBannerImageUrl && oldBannerFilePath) {
            ...
            if (removeBannerError) {
                console.error(
                    "Failed to remove old banner image:",
                    removeBannerError.message
                );
                return NextResponse.json(
                    {
                        error: "Failed to remove old banner image",
                        details: removeBannerError.message,
                    },
                    { status: 500 }
                );
            }
        ...
        const updatedActivity = await updateById(
            "activity",
            activityId,
            updates
        );
        ...
        return new NextResponse(
            JSON.stringify({
                error: "Failed to update activity. Please try again later.",
                message: error.message,
            }),
            {
                status: 500,
