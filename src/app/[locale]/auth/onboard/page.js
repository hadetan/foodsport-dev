...
export default function OnboardPage() {
  ...
  <h1 className="text-2xl font-semibold mb-4">{t('OnboardPage.title')}</h1>
  {error && <div className="alert alert-error mb-4">{error}</div>}
        <form onSubmit={onSubmit} className="space-y-3">
          <div>
            <label className="label"><span className="label-text">{t('OnboardPage.dateOfBirth')}</span></label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="label"><span className="label-text">{t('OnboardPage.weight')}</span></label>
            ...
            <div>
              <label className="label"><span className="label-text">{t('OnboardPage.height')}</span></label>
          ...
          <div>
            <label className="label"><span className="label-text">{t('OnboardPage.district')}</span></label>
            <select
              className="select select-bordered w-full"
              value={form.district}
              onChange={(e)=>setForm(f=>({...f, district:e.target.value}))}
            >
              <option value="">{t('OnboardPage.selectDistrict')}</option>
              {DISTRICTS.map((d) => (
          ...
          <button type="submit" className="btn btn-primary w-full">{t('OnboardPage.finish')}</button>
        </form>
      </div>
    </div>
  );
